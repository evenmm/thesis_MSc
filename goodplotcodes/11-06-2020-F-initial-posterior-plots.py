...
seeds = [1] #[0,11,12,13,17] ## [0,3,5,9,11,12,13,15,19,21] good, 17 mediocre for T=100  [0,11,12,13,17] good for T=1000    1,2,6,8,10,14,20 bad      7,16 mediocre
...
sigma_x_generate_path = 60 # Variance
...
def find_rmse_for_this_lambda_this_seed(seedindex):
    ...
    np.random.seed(0)
    X_initial = (max_neural_tuning_X - min_neural_tuning_X)*np.random.random(T) #X_pca_initial #np.ones(T) #np.linspace(min_neural_tuning_X, max_neural_tuning_X, T) #path
    X_estimate = np.copy(X_initial)

    F_initial = np.log(y_spikes + 0.0008) #np.sqrt(y_spikes) - np.amax(np.sqrt(y_spikes))/2 #np.log(y_spikes + 0.0008)
    ...
    if USE_OFFSET:
        ...
        initial_offset = 0
        offset_optimization_result = optimize.minimize(offset_function, initial_offset, args=(X_estimate, sigma_n, F_estimate, K_gg, x_grid_induce), method = "L-BFGS-B", options = {'disp':True})
...
for i in range(N):
    plt.figure()
    plt.plot(x_grid_for_plotting, observed_mean_spikes_in_bins[i,:], color=plt.cm.viridis(0.1), label="Observed")
    plt.plot(x_grid_for_plotting, h_estimate[i,:], color=plt.cm.viridis(0.5), label="Estimated") 
