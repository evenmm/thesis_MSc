...
# Copy entire spike data for PCA analysis before downsampling
entire_y_spikes = np.copy(binnedspikes)
# And downsample
...
if PCA_TYPE == "angle":
    X_pca_initial = pca_angles
elif PCA_TYPE == "1d":
    X_pca_initial = np.zeros(T)
    for i in range(T):
        X_pca_initial[i] = X_pca_result_1comp[i][0]
if USE_ENTIRE_DATA_LENGTH_FOR_PCA_INITIALIZATION:
    # Crop PCA initialization to selected time interval
    X_pca_initial = X_pca_initial[downsampled_offset:downsampled_offset+T]
...
# Initialize F
# Anscombe transform
#F_initial = 2*np.sqrt(y_spikes + 3/8)
...
if PLOTTING:
    ...
    #plt.ylim((min_neural_tuning_X, max_neural_tuning_X))
    plt.tight_layout()
    plt.savefig(time.strftime("./plots/%Y-%m-%d-%T")+"-peyrache-X-final-L-" + str(x_posterior_no_la(X_estimate, sigma_n, F_estimate, K_gg, x_grid_induce)) + ".png")
