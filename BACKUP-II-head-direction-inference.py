...
def x_posterior_loglikelihood_decoupled_la(U): # Analog to logmargli_gplvm_se_sor_la_decouple.m
    ...
    lowdim_inverse = np.linalg.inv( 1/sigma_n**2 * np.identity(N_inducing_points) + np.matmul(np.transpose(Q), Q) )
    inverse_K_prev = 1/sigma_n**2 * np.identity(T) - 1/sigma_n**2 * np.matmul( np.matmul(Q, lowdim_inverse) , np.transpose(Q) ) #(test this: (X.T.dot(A)*X.T).sum(axis=1) )

    # To find A at X
    Q = np.matmul(K_xg, Kgg_inv_half)
    lowdim_inverse = np.linalg.inv( 1/sigma_n**2 * np.identity(N_inducing_points) + np.matmul(np.transpose(Q), Q) )
    inverse_K_current = 1/sigma_n**2 * np.identity(T) - 1/sigma_n**2 * np.matmul( np.matmul(Q, lowdim_inverse) , np.transpose(Q) ) #(test this: (X.T.dot(A)*X.T).sum(axis=1) )
    ...
    # x prior term
    xTKt = np.dot(np.transpose(X_estimate), K_t_inverse) # Inversion trick for this too? No. If we don't do Fourier then we are limited by this.
...
K_gg += 0.0001*np.identity(N_inducing_points) # This is a bit cheating
fig, ax = plt.subplots()
