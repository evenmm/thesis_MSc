...
TOLERANCE = .01 # for Newton's method
...
plt.savefig(time.strftime("./plots/%Y-%m-%d")+"-headdirection.pdf",format="pdf")
...
while (True):
    ...
    if (max(abs(new_likelihoods - old_likelihoods)) < TOLERANCE):
        break
    old_likelihoods = new_likelihoods

plt.figure()
# Here we plot convergence for neuron 0 
iterationsteps_to_plot = min(iteration,100)
colors = [plt.cm.viridis(t) for t in np.linspace(0, 1, T)]
for t in range(T):
    plt.plot(f_convergence_plot_for_neuron_0[0:iterationsteps_to_plot,t], color=colors[t])
#plt.title('f_i_t Convergence, neuron 0, each line is a chosen timestep')
plt.xlabel("Iteration")
plt.ylabel("Value of f")
plt.savefig(time.strftime("./plots/%Y-%m-%d")+"-convergence.pdf",format="pdf")
...
for n4 in range(N//4):
    ...
    plt.savefig(time.strftime("./plots/%Y-%m-%d")+"hd-fitted-tuning"+str(n4+1)+".pdf",format="pdf")
plt.show()
