...
def find_rmse_for_this_lambda_this_seed(seedindex):
    ...
    if PLOTTING:
        ...
        plt.tight_layout()
        plt.savefig(time.strftime("./plots/%Y-%m-%d")+"-example-plotting-T-" + str(T)  + "-seed-" + str(seeds[seedindex]) + "-path.png")
    ...
    if PLOTTING:
        ...
        plt.savefig(time.strftime("./plots/%Y-%m-%d")+"-example-plotting-true-f.png")
    if PLOTTING:
        ...
        plt.savefig(time.strftime("./plots/%Y-%m-%d")+"-example-plotting-true-h.png")
    ###############################
    ...
    for smoothingwindow_index in range(len(ensemble_smoothingwidths)):
        ...
        if PLOTTING:
            ...
            plt.tight_layout()
            plt.savefig(time.strftime("./plots/%Y-%m-%d")+"-example-plotting-T-" + str(T) + "-lambda-" + str(peak_lambda_global) + "-background-" + str(baseline_lambda_value) + "-seed-" + str(seeds[seedindex]) + "-PCA-initial.png")
        # Initialize X
        np.random.seed(0)
        ...
        if PLOTTING:
            ...
            #plt.ylim((lower_domain_limit, upper_domain_limit))
            plt.tight_layout()
            plt.savefig(time.strftime("./plots/%Y-%m-%d")+"-example-plotting-T-" + str(T) + "-lambda-" + str(peak_lambda_global) + "-background-" + str(baseline_lambda_value) + "-seed-" + str(seeds[seedindex]) + ".png")
        if PLOT_GRADIENT_CHECK:
            ...
            plt.legend(loc="upper right")
            plt.tight_layout()
            plt.savefig(time.strftime("./plots/%Y-%m-%d")+"-example-plotting-T-" + str(T) + "-lambda-" + str(peak_lambda_global) + "-background-" + str(baseline_lambda_value) + "-seed-" + str(seeds[seedindex]) + "-Gradient.png")
            exit()
            """
        ...
        if X_flipped_rmse < X_rmse:
            ...
            if PLOTTING:
                ...
                #plt.ylim((lower_domain_limit, upper_domain_limit))
                plt.tight_layout()
                plt.savefig(time.strftime("./plots/%Y-%m-%d")+"-example-plotting-T-" + str(T) + "-lambda-" + str(peak_lambda_global) + "-background-" + str(baseline_lambda_value) + "-seed-" + str(seeds[seedindex]) + "-flipped.png")
            #############################
            # EM after flipped          #
            #############################
            prev_X_estimate = np.Inf
...
if __name__ == "__main__": 
    ...
    #########################
    result_array = find_rmse_for_this_lambda_this_seed(0)
    seed_rmse_array = result_array[0]
    X_estimate = result_array[1]
    F_estimate = result_array[2]
    y_spikes = result_array[3]
    path = result_array[4]
