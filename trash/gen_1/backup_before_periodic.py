...
path = np.pi + np.pi*np.sin(np.linspace(0,10*np.pi,T)) #numpy.random.multivariate_normal(np.zeros(T), Kt)
...
for i in range(N):
    for t in range(T):
        ...
        if LIKELIHOOD_MODEL == "bernoulli":
            ...
            y_spikes[i,t] = exp(Ht)
true_spike_probability = np.zeros((N, N_plotgridpoints))
true_spike_rate = np.zeros((N,N_plotgridpoints))
for i in range(N):
...
# Initialize X
r = 0.3
X_initial = -0.5 + r * path + (1-r)*np.pi
#np.pi * np.ones(T)
#np.sqrt(path)
#np.pi*np.ones(T)
#r * path + (1-r)*np.pi
...
for iteration in range(N_iterations):
    ...
    # We let F_estimate be equal to the true f by not updating
