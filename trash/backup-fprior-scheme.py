...
lr = 1.00 # Learning rate by which we multiply sigma_n at every iteration
...
if TUNINGCURVE_DEFINITION == "triangles":
    ...
    for i in range(N):
        for t in range(T):
            ...
            if LIKELIHOOD_MODEL == "bernoulli":
                y_spikes[i,t] = exp(true_f[i,t])/(1.+exp(true_f[i,t]))
            elif LIKELIHOOD_MODEL == "poisson":
                y_spikes[i,t] = exp(true_f[i,t])
...
offset = 0
r = 0.3
X_initial = 1 + 0.2*np.random.random(T)
#np.ones(T)
...
for iteration in range(N_iterations):
    ...
    #X_estimate = -1*X_estimate + 2*mean(X_estimate)
plt.show()
...
X_shifted = X_estimate + best_offset
print("Best offset:", best_offset)

print("Comparing L values:")
print("path\t\t",x_posterior_no_la(path))
print("Estimate\t",x_posterior_no_la(X_estimate))
print("Rescaled\t",x_posterior_no_la(X_shifted))

print("Shifted path")
for i in range(-5,5):
    shifted = path + 0.1*i
    print("path shifted by 0.1 *",i,":",x_posterior_no_la(shifted))
print("Shifted scaled estimate:")
for i in range(-5,5):
    shifted = X_shifted + 0.1*i
    print("scaled estimate shifted by 0.1 *",i,":",x_posterior_no_la(shifted))
print("Shifted at path position")
for i in range(-5,5):
    shifted = X_shifted - mean(X_shifted) + mean(path) + 0.1*i
    print("scaled estimate shifted by 0.1 *",i,":",x_posterior_no_la(shifted))



plt.figure()
plt.title("Shifted estimate")
plt.plot(path)
plt.plot(path, color="black", label='True X')
#plt.plot(X_estimate, label='Estimated')
plt.plot(X_shifted, label='Shifted estimate')
plt.legend()
