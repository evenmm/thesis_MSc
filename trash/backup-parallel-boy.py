...
FLIP_AFTER_SOME_ITERATION = True #False
...
baseline_lambda_value = 0.01 #-10 # -2.3 means 10 per cent chance of spiking when outside tuning area.
baseline_f_value = np.log(baseline_lambda_value)
peak_lambda_array = [2] #[4] #[0.01,0.1,0.3,0.5,0.7,1,1.5,2,2.5,3,4,5,6,7,8,9,10]
...
def find_rmse_for_this_lambda_this_seed(seedindex):
    ...
    np.random.seed(0)
    X_initial = np.ones(T) #X_pca_initial #np.ones(T) #2*np.mean(X_pca_initial) - X_pca_initial #np.ones(T) #np.linspace(min_neural_tuning_X, max_neural_tuning_X, T) #path #(max_neural_tuning_X - min_neural_tuning_X)*np.random.random(T)
    ...
    if PLOTTING:
        ...
        #plt.ylim((min_neural_tuning_X, max_neural_tuning_X))
        plt.tight_layout()
        plt.savefig(time.strftime("./plots/%Y-%m-%d")+"-paral-robust-T-" + str(T) + "-lambda-" + str(peak_lambda_global) + "-seed-" + str(seeds[seedindex]) + ".png")
    #############################
    ...
    if X_flipped_rmse < X_rmse:
        ...
        if PLOTTING:
            ...
            #plt.ylim((min_neural_tuning_X, max_neural_tuning_X))
            plt.tight_layout()
            plt.savefig(time.strftime("./plots/%Y-%m-%d")+"-paral-robust-T-" + str(T) + "-lambda-" + str(peak_lambda_global) + "-seed-" + str(seeds[seedindex]) + "-flipped.png")
        prev_X_estimate = np.Inf
        for iteration in range(N_iterations):
            if iteration > 0:
                sigma_n = sigma_n * lr  # decrease the noise variance with a learning rate
    ...
    if PLOTTING:
        ...
        #plt.ylim((min_neural_tuning_X, max_neural_tuning_X))
        plt.tight_layout()
        plt.savefig(time.strftime("./plots/%Y-%m-%d")+"-paral-T-" + str(T) + "-lambda-" + str(peak_lambda_global) + "-seed-" + str(seeds[seedindex]) + "-final.png")
